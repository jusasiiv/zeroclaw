...
/// Request timeout (300s) — prevents slow-loris attacks while allowing LLM workloads
...
/// Simple chat without tools (kept for lightweight/testing use cases).
#[allow(dead_code)]
...
async fn run_webhook_chat_stateful(state: &AppState, message: &str) -> anyhow::Result<String> {
    ...
    let mut history = {
        ...
        if !had_prior_history {
            ...
            if !native_tools {
                system_prompt
                    .push_str(&crate::agent::loop_::build_tool_instructions(&tools_registry, None));
            }
